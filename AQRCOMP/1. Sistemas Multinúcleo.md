...
- Toma decisiones: **registros, direccionamiento d ememeoria, operandos de las isntrucciones, tipos de isntrucciones, etc.**
...
La **segmentación** es la ejecución de instrucciones en **etapas,** de manera que cada etapa tiene un **retardo adicional** (para almacenar los resultados de al etapa en registros).

La **profundidad de segmentación** es el número de **etapas de segmentación**.

$F04$ (*Fan_out of 4*) es una unidad de tiempo equivalente el retardo que tiene una señal al atrevesar un único inversor con 4 inversores conectados a la salida. Su valor exacto depende de la tecnología de fabricación, es decir, del **nodo tecnológico**.
...
Hay un **límite** en las condiciones de **temperatura** en las que puede funcionar un circuito. Se hay llegado a un **límite de disipación** de potencia (100-200W) debido a cuestiones técnicas y económicas. Mantener el límite en cada nueva generación de microprocesadores requiere **optimizar el consumo de potencia**.
...
Hasta 2003 se usó un modelo **mononúcleo** de procesamiento **superescalar**, es decir, ejecución de varia sinstrucciones a la vez en cada ciclo:
