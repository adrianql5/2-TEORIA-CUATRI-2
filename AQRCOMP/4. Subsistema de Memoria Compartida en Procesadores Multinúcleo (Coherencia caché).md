...
**Consistencia:** define el **orden** en que se ejecutan las operaciones de memorias unas respecto a otras y, por tanto, que esas operaciones se realicen en el orden del **programa secuencial**.
...
- **Escritura con Invalidación** (WI o write-invalidate): cuando se realiza una escritura, se distribuye una **señal de invalidación** a través del bu y todas las cachés comprobarán si tienen un copia de ese dato. Las que lo tengan, invalidarán la línea que lo contiene.
...
## 4.3.2 MSI, Protocolo de Invalidación de 3 Estados
...
- **Falsa compartición (false sharing):** se produce cuando dos **variables sin relación** entre ellas se encuentran dentro del **mismo bloque cache**. Como consecuencia, **todo el bloque** se retransmite entre procesadores, aunque se esté accediendo a diferentes variables
- Fallos de **compartición verdadera (true sharing):** un procesador escribe en un bloque $S$
- Fallos de **falsa compartición:** un procesador escribe en un bloque $S$ y lo invalida, luego otro procesador lee una **palabra diferente** del bloque

## 4.3.3 MESI, Protocolo de Invalidación de 4 Estados
En el procotolo MSI, si un proceso escribe en un bloque de su caché, se lanza al bus una **señal de invalidación** de las copias de otras cachés, **aunque no las haya**. Esto produce una **degradación** de las aplicaciones que no comparten variables entre procesos, pues la señal de invalidación se envía de todas formas al bus.
...
### Acciones generadas por el procesador
![[Pasted image 20250527174715.png]]
![[Pasted image 20250527174746.png]]

| **Evento**                             | **Causa**                                                                 | **¿Qué pasa en la propia caché?**                                                                                                                     | **¿Qué provoca la petición en otras cachés y en la MP?**                                                                                                                                                                                |
|---------------------------------------|---------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **FALLO DE LECTURA**                  | El proceso lee (PrLec) pero el bloque **no está** en la caché.            | 1. El controlador difunde la **PtLec**. <br> 2. – Si hay copias del bloque en otras cachés → el bloque pasa a **S**. <br> – **Si no hay copias del bloque en otras cachés → el bloque pasa a E** | – Si el bloque está **M en otra caché**: <br> 1. La caché deposita el bloque en el bus. <br> 2. La caché coloca el paquete **RpBloque** en el bus. <br> 3. La caché pasa el bloque a **S**. <br> 4. La **MP** recoge el bloque y lo pasa a válido. <br> – Si el bloque está **S en otra caché**: <br> 1. La **MP** deposita el bloque en el bus. <br> 2. El bloque sigue como **S**. <br> – Si el bloque está **E en otra caché**: <br> 1. La caché pasa el bloque a **S**. <br> 2. La **MP** coloca el bloque en el bus. |
| **FALLO DE ESCRITURA**                | El proceso escribe (PrEsc) pero el bloque **no está** en la caché.        | 1. El controlador difunde la **PtLecEx**. <br> 2. El bloque pasa a **M**.                                                                               | – Si el bloque está en **M en otra caché**: <br> 1. **Bloquea la lectura de memoria.** <br> 2. La caché deposita el bloque en el bus. <br> 3. La caché pasa el bloque a **I**. <br> – Si el bloque está en **S en otra caché**: <br> 1. La caché pasa el bloque a **I**. <br> – **Si el bloque está en E en otra caché:** <br> 1. **La caché pasa el bloque a I.**        |
| **ACIERT0 DE LECTURA**                | El proceso lee (PrLec) y el bloque está en la caché en no **I**.          | El estado se mantiene y no se genera petición.                                                                                                         | 1. La **MP** deposita el bloque en el bus.                                                                                                                                                                                                |
| **ACIERT0 DE ESCRITURA EN BLOQUE S** | El proceso escribe (PrEsc) y el bloque está en la caché en **S**.         | 1. El controlador difunde la **PtLecEx**. <br> 2. El bloque pasa a **M**.                                                                               | 1. La **MP** deposita el bloque en el bus. <br> ↳ Lo tiene **válido seguro** (aunque se ignora). <br> 2. La memoria pasa el bloque a inválido. <br> – Si el bloque está en **S en otra caché**: <br> 1. La caché pasa el bloque a **I**.       |
| **ACIERT0 DE ESCRITURA EN BLOQUE M** | El proceso escribe (PrEsc) y el bloque está en la caché en **M**.         | El estado se mantiene y no se genera petición.                                                                                                         |                                                                                                                                                                                                                                          |
