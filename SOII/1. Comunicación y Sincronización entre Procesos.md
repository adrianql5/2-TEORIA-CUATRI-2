...
Este problema se puede **generalizar** par aun número arbitrario de productores y consumidores **sin cambiar el código**.
...
- `up(semaforo)`: incrementa el valor del `semaforo` 
	- Si algún proceso estaba inactivo en ese `semaforo`, sin poder completar una operación `down` anterior, el sistema selecciona uno de ellos al azar y permite que complete la operación `down`. Así, después de la operación `up` en un `semáforo` con procesos dormidos, este seguirá en `0` pero habŕa un proceso menos dormido en él.
